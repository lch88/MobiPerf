...
public class FSKModule {
    ...
    public static int FREQUENCY_HIGH = 3000;
    public static int FREQUENCY_LOW = 2500;
    //high: 7 samples/peak
    //low : 14 samples/peak
    // 1492 samples/message low+8bits+stop+end
    // 136 samples/bit  (=1492/11)
    private static int SAMPLES_PER_BIT = 136 * 100;
...
//-----------------------------------------
    ...
    private int decodeUniqueMessage(int[] bits, double[] sound, int[] nPeaks){
        ...
        if (index + 8 + 2 > bits.length)
            throw new AudioTestException("Message cutted, start bit at " + index, AudioTestException.TYPE_FSK_DECODING_ERROR);
